...
from ASB_app.service import adastra_service
...
class SNPItem(Resource):
    ...
    def get(self, rs_id, alt):
        ...
        """
        try:
            return adastra_service.get_full_snp(rs_id, alt)
...
class SearchSNPByIdCollection(Resource, PaginationMixin):
    BaseEntity = adastra_service.SNP
    ...
    def get(self, rs_id):
        ...
        filters = adastra_service.get_filters_by_rs_id(rs_id)
...
class SearchSNPByGeneIdCollection(Resource, PaginationMixin):
    ...
    def get(self, gene_id):
        ...
        filters = adastra_service.get_filters_by_gene(get_gene_by_id(gene_id))
        result = self.paginate(all_args, extra_filters=filters)
...
class SearchSNPByGeneNameCollection(Resource, PaginationMixin):
    ...
    def get(self, gene_name):
        ...
        if gene is None:
            return {'results': [], 'gene': None, 'total': 0}
        filters = adastra_service.get_filters_by_gene(gene)
        result = self.paginate(all_args, extra_filters=filters)
...
class AdvancedSearchSNP(Resource, PaginationMixin):
    ...
    def get(self):
        ...
        try:
            filters = adastra_service.construct_advanced_filters(all_args)
            result = self.paginate(all_args, extra_filters=filters)
            return {'results': result, 'total': self.items_count(extra_filters=filters)}
        except ParsingError:
            api.abort(400)
...
class AdvancedSearchSNPCSV(Resource):
    ...
    def get(self):
        ...
        """
        try:
            return adastra_service.get_snps_by_advanced_filters_tsv(search_parser.parse_args())
        except ParsingError:
            api.abort(400)
...
class TransctiptionFactorBrowse(Resource, PaginationMixin):
    BaseEntity = adastra_service.TranscriptionFactor
...
class CellLineBrowse(Resource, PaginationMixin):
    BaseEntity = adastra_service.CellLine
...
class FrontPageStatistics(Resource):
    ...
    def get(self):
        return adastra_service.get_overall_statistics()
...
class TranscriptionFactorHint(Resource):
    ...
    def get(self):
        ...
        return adastra_service.get_hints('TF', args.get('search', ''), args.get('options', []))
...
class CellLineHint(Resource):
    ...
    def get(self):
        args = used_hints_parser.parse_args()
        return adastra_service.get_hints('CL', args.get('search', ''), args.get('options', []))
...
class GeneNameHint(Resource):
    ...
    def get(self):
        args = used_hints_parser.parse_args()
        return adastra_service.get_hints_for_gene_name(args.get('search', ''))
...
class SNPItemCSV(Resource):
    ...
    def get(self, rs_id, alt, what_for):
        ...
        try:
            return adastra_service.get_full_snp_tsv(what_for, rs_id, alt, args['columns'])
        except NoResultFound:
            api.abort(404)
